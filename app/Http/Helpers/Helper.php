...
if (!function_exists('eventDates')) {
  function eventDates($event_id)
  {
    ...
    $event_dates = EventDates::where('event_id', $event_id)->whereDate('end_date_time', '>=', $now)->orderBy('start_date_time', 'asc')->get();
    return $event_dates;
  }
}
...
if (!function_exists('eventExpDates')) {
  ...
  {
    $now = Carbon\Carbon::now()->format('Y-m-d h:i:s');
    $event_dates = EventDates::where('event_id', $event_id)->whereDate('end_date_time', '<=', $now)->orderBy('start_date_time', 'desc')->get();
    return $event_dates;
  }
}
